...
| Date | State |
|---|---|
| 2026-09-22 | **Assessments dropdown stays open long enough to click (branch `claude/youthful-cerf-b0d7ci`).** webs reported the menu closing before she could reach a link. Two causes in `Navigation.jsx`/`.css`: a 0.35rem gap between the button and the menu fired `mouseleave` on the way down, and a click after hovering toggled the hover-opened menu straight back closed. Fixed with an invisible bridge over the gap (desktop only), a 300ms close delay on leave, and a click that keeps a hover-opened menu open. Keyboard (Enter opens, Escape closes and returns focus) unchanged. Checked in Chromium: slow move to a link, click navigates, hover-then-click stays open, close after the delay. Build + lint clean. |
| 2026-09-22 | **Blog post page and Drive sync fixes, from webs's review of the first Q&A draft (branch `claude/youthful-cerf-b0d7ci`).** (1) **Title and slug come from inside the Doc:** the first Heading 1 is the title, a Heading 2 directly under it is the subtitle, and a `Slug:` line sets the address; the Doc name is only a fallback. A changed slug renames the post file and clears the old image folder. (2) **Bold and italics survive the sync:** Google's HTML export marks them with generated CSS classes, which Turndown dropped; the script now reads the export's style block. (3) **Cover image replaces the vine texture** as the hero background, under a fixed deep-olive scrim that keeps the hero text above 4.5:1 in both themes. (4) **Body spacing fixed:** `.blog-post-body p { margin: 0 }` was overriding the block gap, so paragraphs ran together; markdown now renders inside `.blog-prose` with scoped rules. (5) **Q&A layout:** paragraphs starting `Q:` and `A:` render as a labeled question card and an answer section, with a short unpunctuated last line shown as the sign-off. (6) **Tag chips fixed:** the body's list rules were indenting them and pushing the second chip down. Template and pipeline docs updated. Build + lint clean; checked light, dark, and mobile. **webs actions:** update the `_TEMPLATE` Google Doc to match `docs/blog-post-template.md`, and add a `Slug:` line to the Green in Name Only Doc. The Action runs from `main`, so these changes need a PR into `main` before the next sync uses them. |
| 2026-09-02 | **Field-guide PDFs rebuilt: on-brand, accessible, voice-passed (branch `claude/uc-autonomy-procedures-bb89gk`).** The four `public/field-guides/*.pdf` (Before Fire Season, Smoke Signals, When the Canopy Burns, After the Firestorm) were stale exports with an old, bracketed `[understorycollab.com/quiz]` CTA. Regenerated from webs's updated Google Docs (the "1-Brush fire … 4-firestorm" folder) through a Chromium print pipeline. **Design:** white pages (no cream frame — Chromium can't paint page margins, so full-bleed would cost per-page margins + page numbers), Overpass embedded, per-paper severity accent (brush→firestorm) darkened to clear AA as text, callout cards (icon + bold header + list, 2-column at 4+ points), practice cards for bold-lead runs, a per-section takeaway box, icons only on callouts/CTA/About (no repeated mark), footer page numbers. **Accessible:** tagged PDF (StructTreeRoot), `lang=en`, AA contrast on all text, semantic h1/h2/h3, severity by label + colour. **Voice pass (webs's calls: Title Case kept, names kept):** replaced Sarah/Marcus with Raj/Wei (Carol, Dave already in her diverse-names list); removed false-contrast lines, the "honest" virtue word, and sentence-opening And/But/So; split the worst walls of text. **webs must:** proofread the four PDFs against her Docs (text was regenerated, not edited in place). The build script + working `.md` live in the session scratchpad, not the repo. Not yet deployed; ships with the pending PR into `preview`. |
| 2026-09-01 | **Consent + provider cleanup: Kit dropped; contact goes list-free, the field guide keeps its double opt-in capture (branch `claude/uc-autonomy-procedures-bb89gk`).** webs confirmed MailerLite only, Kit dead (it never alerted on new submissions), Resend working. (1) **`api/contact.js` rewritten to Resend-only:** every valid submission emails contact@understorycollab.com (reply-to the sender), with no Kit and no list write, because writing in is not a newsletter opt-in. Resend is now REQUIRED (503 if unconfigured, 502 on send failure); honeypot, validation, and the subject header-injection guard are kept. (2) **`api/field-guide.js` keeps its MailerLite capture** into the assessment group. The field guide exists to capture emails, and the consent bar is met by MailerLite double opt-in, not by withholding the capture. (An earlier pass in this branch briefly removed the add; reverted after webs clarified the purpose, so `field-guide.js` and the `Quiz.jsx` capture copy are back to their prior state.) (3) **`Privacy.jsx` "Where Your Information Goes" corrected:** it wrongly said contact goes to a Google Sheet and the newsletter runs on Kit; now Resend email with no list, and MailerLite. (4) **Docs:** `.env.example` and the README Forms table rewritten (Kit removed; newsletter and the field guide both add to MailerLite via double opt-in; contact and the field-guide email need Resend). Build + lint clean. **Confirmed by webs:** MailerLite "Double opt-in for API and integrations" is ON, so API-added subscribers (field guide + newsletter box) get a confirmation email and are not listed until they confirm. **webs action:** verify the MailerLite sender email, or the confirmation email will not send. **Privacy pass done (2026-09-01):** `Privacy.jsx` rewritten to disclose every real data flow — Resend (contact message + guide email), MailerLite (newsletter + assessment lists, double opt-in), Google (the Q&A Google Form + Google Calendar booking), and Stripe (office-hours payments, card details go straight to Stripe) — with a collect/use/retention refresh and a note that office hours are sold through the business entity Webs on the Webs, LLC, so that name can appear on receipts. **Confirmed by webs:** Understory Collaborative is a DBA of Webs on the Webs, LLC; `Privacy.jsx` now states that outright. **webs to confirm:** whether GDPR/CCPA-specific rights language is wanted. **Stripe branding (open):** there is no separate Understory Stripe account yet, so office-hours charges show "Webs on the Webs" / WEBSONTHEWEBS.COM. Fix depends on whether that one account charges for anything besides Understory office hours (rename its public name + descriptor to Understory) or not (create a separate Understory account under the org and reconnect the Google Calendar booking). Not yet deployed; needs a PR into `preview`. |
...
| 2026-08-20 | **Texture went geometric (same branch, per webs "lean into something more geometric").** Replaced the leafy-vine tile with a geometric canopy lattice: a hexagon honeycomb with a leaf-lens centered in each cell (line-art, seamless, generated on a triangular grid). Crisp like studio w labs' hexagons but botanical and distinct, and calmer than a competing overlapping-circle "flower" lattice that was tried and rejected as too busy. Same mask technique and `--texture-ink` theming; mask-size 128px, opacity 0.06. Comment in `src/index.css` updated. Build + lint clean; checked the hero band (light) and the assessment act card (dark). Warm accent from the prior entry still in place and still awaiting webs's keep/drop call. |
| 2026-08-20 | **Texture pulled; webs is designing her own tile.** She rejected both the vine and the geometric hex-leaf watermark (reads as AI slop), but likes the colors/warm accent. Disabled the texture layer with a single `display: none` on the shared `::before` block in `src/index.css` (bands are flat now); the `--texture-canopy` token and the mask machinery are left intact so her tile drops straight in. **Handoff spec given to webs:** one-color seamless SVG (or transparent hi-res PNG) line-art, sparse (sits at ~6% opacity), note the repeat size. To restore: put her art in `--texture-canopy`, delete the `display: none`, set mask-size to her tile. Build + lint clean. |
| 2026-08-20 | **webs's own hand-drawn vine is now the texture.** She drew a vine field in Procreate (transparent PNG, 2048², olive linework). Downscaled to 1024² and kept only the alpha channel as `public/vine-texture.png` (236 KB); `--texture-canopy` points at it and the `::before` layer is back on. Because it is a full illustration (not a small tile) it is applied `mask-size: cover; no-repeat; center` — no seam, one drawn backdrop per band — at 0.08 opacity, still themed olive via `--texture-ink`. Reads as real hand-made personality on the hero band and the assessment card, both themes. **Minor, deferred:** two faint smudge specks remain in her source art (~mid-canvas, mirrored); invisible at 0.08, worth cleaning only if the texture is ever used larger/darker. Build + lint clean. |
...
| 2026-08-20 | **Proper seed / sprout / tree icons in "How we work".** Pulled webs's three official icons from the Drive images folder (`noun-seed-6474297`, `noun-sprout-7907397`, `noun-tree-201654`) into `src/assets/` and mapped them Root=seed, Rise=sprout, Flourish=tree, so the row now reads as a real growth progression matching the copy (foundation → building → thrive). Replaces the older germination/seedling/leafy-tree set. **Open flag for webs:** the hero canopy mark still uses the older leafy tree (`noun-tree-with-leaves`); switch it to the proper tree for one consistent tree across the page, or keep the airier leafy one there? The now-unused `noun-germination`/`noun-seedling` assets are left in place. Build + lint clean; checked both themes. |
| 2026-08-20 | **Nav mark and hero focal swapped (per webs).** The nav home button is now the tree line-art (`noun-tree-with-leaves`) rendered as a cream mask next to the wordmark, replacing the UC badge. The hero focal is now the **framed UC logo**: the badge sits in a matte frame (`.home-hero-logo` — card surface, hairline border, soft olive shadow) in place of the olive tree emblem. This also resolves the earlier "unify the hero tree" flag — the tree now lives only in the nav and the Flourish step, the badge is the hero. Same responsive behavior (framed logo hidden on phones so the headline owns the width). Build + lint clean; both themes checked. Icon-sizing retune, warm accent, and vine texture all still in place. |
