...
| 2026-08-14 | Q&A backend decision: **custom UI posts to a Google Form** webs owns (form id `1FAIpQLSd5HTS0VYZR4NDR5iRnz1Ecg3gUeJ0-un-45Pfs8bLmbb9i6Q`, copied from studio w labs). Updated Q&A fields to match: what are you stuck on (req), what's the product, stage (idea/building/launched/growing), can we share (req: name/anonymous/private), name, email (**now required**, anti-spam). PENDING to finish wiring: (1) webs turns OFF built-in email collection and adds Email as a normal required short-answer question, else formResponse can't submit; (2) webs sends a pre-filled link so we get each field's `entry.<id>`; (3) rework `api/questions.js` to POST urlencoded `entry.*` to the form's `/formResponse` endpoint (currently posts JSON). |
