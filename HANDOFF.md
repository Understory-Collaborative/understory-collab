...
| Date | State |
|---|---|
| 2026-09-01 | **Consent + provider cleanup: Kit dropped, no list-add on the guide or contact (branch `claude/uc-autonomy-procedures-bb89gk`).** webs confirmed MailerLite only, Kit dead (it never alerted on new submissions), Resend working. (1) **`api/contact.js` rewritten to Resend-only:** every valid submission emails contact@understorycollab.com (reply-to the sender), with no Kit and no list write. Resend is now REQUIRED (503 if unconfigured, 502 on send failure); honeypot, validation, and the subject header-injection guard are kept. (2) **`api/field-guide.js` no longer adds anyone to MailerLite:** the guide is delivered by the instant on-page download plus an optional Resend email, so a valid request always succeeds and no longer depends on the list key. Removed the `addSubscriber` / `MAILERLITE_GROUPS` import and the "we've also added you to our list" email line. (3) **Copy fixed to match:** `Quiz.jsx` guide-capture and success text drop "confirm your subscription" / "unsubscribe anytime"; `Privacy.jsx` "Where Your Information Goes" corrected — it wrongly said contact goes to a Google Sheet and the newsletter runs on Kit, now Resend email with no list, and MailerLite. (4) **Docs:** `.env.example` and the README Forms table rewritten (Kit removed; newsletter is the only MailerLite path; contact and the field-guide email need Resend). The newsletter box (`api/subscribe.js`, double opt-in) is unchanged and stays the only list-growth path. Build + lint clean. **webs must confirm on live:** the MailerLite double opt-in toggle is ON. **Open (flagged, not changed):** the Privacy policy still omits the office-hours booking processors (Google Calendar + Stripe); a fuller privacy review is worth doing. Not yet deployed; needs a PR into `preview`. |
